...
#4. get list university in US from wiki page: 
...
class HyperlinkParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == "table" and "href" in attrs:
...
def crawl(url, save_path):
    if os.path.exists(save_path):
        os.remove(save_path)
    url = "https://en.wikipedia.org/wiki/List_of_colleges_in_Canada"
    tables = pa.read_html(url)
    publicCaTable = tables[1]
    publicCaTable.to_csv('privateCaUniversity.csv', index=False, encoding="utf-8-sig")
